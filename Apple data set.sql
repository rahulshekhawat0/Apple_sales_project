...
select ss.store_name, sum(quantity), s.store_id as total_units
from sales s 
join stores ss
on s.store_id = ss.store_id
group by store_name,3
order by 2 desc;
...
--4. Determine how many stores have never filed a warranty claim.
...
select count(claim_id) as  warranty_claim from warranty 
...
select count(claim_id) as warranty_claims  FROM warranty as w
LEFT JOIN 
sales as s
ON s.sale_id = w.sale_id
WHERE 
...
select count(claim_id) as warranty_claims, extract(year from launch_date), product_name  FROM warranty as w
...
select sum(quantity)as amount,TO_CHAR(sale_date, 'MM-YYYY') as month
from sales s
join stores st
on s.store_id = st.store_id
...
SELECT 
	...
	total_unit_sold,
	total_claim,
	round(COALESCE(total_claim::numeric/total_unit_sold::numeric * 100, 0),2)
	as risk
FROM
(SELECT 
	st.country,
...
--19. Identify the store with the highest percentage of "Paid Repaired" claims relative to the total claims filed.
WITH paid_repair
AS
(SELECT 
	s.store_id,
